...
async function renderRoute(){
  ...
  try{await renderers[route](root,ctx,params);root.focus({preventScroll:true});window.scrollTo({top:0,behavior:'smooth'});if(params.has('action')){params.delete('action');const query=params.toString();history.replaceState(null,'',`#${route}${query?`?${query}`:''}`);}}catch(err){console.error(err);root.innerHTML=`<section class="page"><div class="empty-state">Não foi possível abrir esta área.<br>${escapeHTML(err.message||String(err))}</div></section>`;toast('Ocorreu um erro ao abrir a área.','error');}
}
...
function showOnboarding(){
  ...
  const render=()=>{
    ...
    if(step===0)body.innerHTML=`<span class="eyebrow">GENESIS ${APP_VERSION}</span><h2>Bem-vindo ao Assistente Pessoal VIP</h2><p>Esta instalação começará vazia e armazenará seus dados neste navegador. A estrutura inicial inclui AGORA, QG, GÊMEO FINANCEIRO, MEMÓRIA, ORÁCULO, FORJA, VIP GUIA, GUARDIÃO e CONFIGURAÇÕES.</p><div class="grid grid-2"><div class="kv"><small>FUNCIONAMENTO</small><strong>Offline-first</strong></div><div class="kv"><small>IDENTIDADE</small><strong>Azul-marinho e verde metálico</strong></div></div><div class="setup-actions"><span></span><button id="setupNext" class="btn btn-primary">Começar</button></div>`;
