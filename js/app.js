...
const renderers={agora:renderAgora,qg:renderQG,memoria:renderMemoria,oraculo:renderOraculo,forja:renderForja,guardiao:renderGuardiao,configuracoes:renderConfiguracoes};
let settings={};
...
function buildNavigation(){
  const desktop=['agora','qg','memoria','oraculo','forja','guardiao','configuracoes'];
...
function showOnboarding(){
  ...
  const render=()=>{
    ...
    if(step===0)body.innerHTML=`<span class="eyebrow">GENESIS ${APP_VERSION}</span><h2>Bem-vindo ao Assistente Pessoal VIP</h2><p>Esta instalação começará vazia e armazenará seus dados neste navegador. A estrutura inicial inclui AGORA, QG, MEMÓRIA, ORÁCULO, FORJA, GUARDIÃO e CONFIGURAÇÕES.</p><div class="grid grid-2"><div class="kv"><small>FUNCIONAMENTO</small><strong>Offline-first</strong></div><div class="kv"><small>IDENTIDADE</small><strong>Azul-marinho e verde metálico</strong></div></div><div class="setup-actions"><span></span><button id="setupNext" class="btn btn-primary">Começar</button></div>`;
